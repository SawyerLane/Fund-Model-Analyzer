...
with st.sidebar.expander("💼 Capital Deployment"):
    ...
    num_tranches = st.number_input("Number of Debt Tranches", min_value=0, max_value=5, value=2, help="Number of distinct debt facilities for the fund.")
    debt_tranches_data = []
    for i in range(num_tranches):
        st.markdown(f"**Tranche {i+1}**")
        amount = st.number_input(f"Amount {i+1} ($)", min_value=1_000_000, value=10_000_000, step=1_000_000, key=f"d_amt_{i}", help="Principal amount of this debt tranche.")
        annual_rate = st.number_input(f"Annual Rate {i+1} (%)", min_value=0.0, max_value=20.0, value=6.0, step=0.1, key=f"d_rate_{i}", help="Annual interest rate for this tranche.")
        interest_type = st.selectbox(f"Interest Type {i+1}", ["Cash", "PIK"], key=f"d_int_type_{i}", help="Cash interest is paid monthly; PIK (Payment-in-Kind) interest is added to the principal.")
        drawdown_start = st.number_input(f"Drawdown Start {i+1} (Month)", min_value=1, value=1, key=f"d_start_{i}", help="The month the fund starts drawing down this debt.")
        drawdown_end = st.number_input(f"Drawdown End {i+1} (Month)", min_value=1, value=24, key=f"d_end_{i}", help="The month the fund finishes drawing down this debt.")
        maturity_month = st.number_input(f"Maturity {i+1} (Month)", min_value=1, value=120, key=f"d_maturity_{i}", help="The month the debt principal must be repaid.")
...
with st.sidebar.expander("💵 Economics & Fees"):
    asset_yield = st.number_input("Asset Yield (Annual %)", min_value=0.0, max_value=50.0, value=9.0, step=0.1, help="Annual yield generated by the fund's income-producing assets.") / 100
    asset_income_type = st.selectbox("Asset Income Type", ["Cash", "PIK"], index=1, help="How asset yield is realized (Cash or PIK).")
    equity_for_lending_pct = st.slider("Equity Portion for Asset Yield (%)", min_value=0.0, max_value=100.0, value=100.0, step=1.0, help="Percentage of deployed equity that forms the base for the Asset Yield, alongside debt.") / 100.0
...
try:
    ...
    flow_cols = ['LP_Contribution', 'GP_Contribution', 'LP_Distribution', 'GP_Distribution', 'Asset_Interest_Income']
    ...
    with st.expander("View Annual Summary"):
        display_cols = ['Assets_Outstanding', 'Equity_Outstanding', 'Debt_Outstanding', 'Unused_Capital', 'Asset_Interest_Income', 'LP_Contribution', 'LP_Distribution', 'Annual_LP_Net_Cash_Flow']
