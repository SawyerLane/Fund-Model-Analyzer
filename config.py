...
class ExitYearConfig:
    ...
    year: int
    pct_of_portfolio_sold: float = 1.0  # 0..1
...
class DebtTrancheConfig:
    ...
    amortization_period_years: int = 30  # informational, used when we schedule equal payments
    ...
    def __post_init__(self):
        ...
        if self.drawdown_end_month < self.drawdown_start_month:
            raise ValueError(
                f"Drawdown end month ({self.drawdown_end_month}) must be >= start month ({self.drawdown_start_month})")
        if self.maturity_month < self.drawdown_end_month:
            raise ValueError(
                f"Maturity month ({self.maturity_month}) must be >= drawdown end month ({self.drawdown_end_month})")
...
class FundConfig:
    ...
    equity_for_lending_pct: float = 0.0  # 0..1
    ...
    # Auto-scaling of debt draws to target LTV on lending book
    ...
    def __post_init__(self):
        ...
        if abs(total_partner_commitment - self.equity_commitment) > 1e-6:
            raise ValueError(
                f"LP ({self.lp_commitment}) + GP ({self.gp_commitment}) must equal equity commitment ({self.equity_commitment})")
        ...
        for i, tranche in enumerate(self.debt_tranches):
            if tranche.maturity_month > max_fund_months:
                raise ValueError(
                    f"Debt tranche {i+1} maturity month ({tranche.maturity_month}) exceeds fund duration ({max_fund_months} months)")

        if self.target_ltv_on_lending < 0.0 or self.target_ltv_on_lending >= 0.95:
            raise ValueError(f"Target LTV must be between 0% and <95%, got {self.target_ltv_on_lending:.2%}")
